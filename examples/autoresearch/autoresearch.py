...
Four things the platform does carry the whole demo:
    ...
    run()       each turn is one exec inside an agent's VM, into a shell that
                keeps its environment and cwd between calls — and across forks
...
from helper import (AGENTS, INSTALL_AGENT, INSTALL_NODE, INSTALL_TORCH, INSTALL_UV,
                    EXEC_MARK, PROMPT, RUN, SETUP_SHELL, TASK, TURNS, VERIFY_TORCH,
...
def prepare_vm(ark: Arker) -> VM:
    ...
    # Session 0 is a stateful shell: PATH, HOME and cwd set once here hold for
    # every run below, and for every agent forked off prep.
    prep.run(SETUP_SHELL)
...
def run_agent(ark: Arker, prep: VM, n: int, vgpu: float) -> list[dict]:
    ...
    # secret, spliced into the Authorization header on the way out to
    # openrouter.ai. Inside the machine the key does not exist — `pi` sees only
    # the placeholder from SETUP_SHELL. The trailing allow is required: a
    # non-empty document denies every flow it does not match, and the agent
    # still needs the rest of the internet.
    vm = ark.fork(
        source=prep,
        name=f"agent{n}",
        vgpu=vgpu,
