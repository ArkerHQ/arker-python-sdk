...
// ── Arker (control plane) ──────────────────────────────────────────
await call("Arker.list (vms)", () => arker.list());
await call("Arker.filesystems.list", () => arker.filesystems.list(), { stubOk: true });
...
await call("Computer.get", () => computer.get());
...
// ── Runs ────────────────────────────────────────────────────────────
const runs = await call("Runs.list", () => computer.runs.list()) as any;
const someRun = bg?.runId ?? runs?.runs?.[0]?.run_id;
if (someRun) {
  await call("Runs.get", () => computer.runs.get(someRun));
  await call("Runs.cancel", () => computer.runs.cancel(someRun), { stubOk: true });
} else { rec("STUB", "Runs.get/cancel", "no run id available"); }

// ── Sessions ────────────────────────────────────────────────────────
await call("Sessions.list", () => computer.sessions.list());
const sess = await call("Sessions.create", () => computer.sessions.create({})) as any;
const sid = sess?.session_id;
if (sid) {
  await call("Sessions.get", () => computer.sessions.get(sid));
  await call("Sessions.delete", () => computer.sessions.delete(sid));
} else { rec("STUB", "Sessions.get/delete", "no session id"); }

// ── Syncs ───────────────────────────────────────────────────────────
const path = `/home/user/${stamp}.txt`;
await call("Syncs.writeFile", () => computer.syncs.writeFile(path, `${stamp}\n`));
await call("Syncs.readFile", async () => {
  const b = await computer.syncs.readFile(path);
  const got = new TextDecoder().decode(b);
  if (got !== `${stamp}\n`) throw new Error(`readback mismatch: ${JSON.stringify(got)}`);
  return b;
});
await call("Syncs.list", () => computer.syncs.list(), { stubOk: true });
await call("Syncs.get", () => computer.syncs.get("sync_does_not_exist"), { stubOk: true });
await call("Syncs.delete", () => computer.syncs.delete("sync_does_not_exist"), { stubOk: true });

// ── Tunnels ─────────────────────────────────────────────────────────
await call("Tunnels.list", () => computer.tunnels.list(), { stubOk: true });
await call("Tunnels.get", () => computer.tunnels.get(8080), { stubOk: true });
await call("Tunnels.delete", () => computer.tunnels.delete(8080), { stubOk: true });

// ── Filesystems get/delete ──────────────────────────────────────────
await call("Filesystems.get", () => arker.filesystems.get("fs_does_not_exist"), { stubOk: true });
await call("Filesystems.delete", () => arker.filesystems.delete("fs_does_not_exist"), { stubOk: true });
