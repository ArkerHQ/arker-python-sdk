...
export interface paths {
    ...
    "/v1/vms/{id}/policies": {
         ...
         * @description Replace the complete network policy of a mutable VM owned by the authenticated caller and apply it to the running VM. Public template VMs are immutable.
         */
...
export interface components {
    schemas: {
        ...
        PolicyDoc: {
            ...
            /** @description Nullable base `.app` hostname for inbound services. The bare hostname selects VM port 8080. For another port, insert `-<port>` before the first dot: `<vm>-<port>.<region>.arker.app`. Clients must not require this field to be present. */
            readonly hostname?: string | null;
        ...
        Vm: {
            ...
            /** @description GPU sizing bounds for each GPU platform this VM can be forked onto; absent when it has no GPU platform. A source VM may be offered on several GPU platforms at once, so this is a list — pick the entry matching the platform you intend to request. */
        ...
        RunRequest: {
            ...
            /** @description Zero-based session index within the VM. Selects a shell SLOT on the VM, FIND-OR-CREATE: if no session occupies this index one is created. Use distinct indexes to run commands CONCURRENTLY on one VM — two runs targeting the same session are serialised. Omitting BOTH `session_id` and `session_idx` targets index 0, the VM's default shell, so unrelated callers that both omit them share one shell and therefore one working directory and environment. */
        ...
        NetworkInput: {
            /** @description OpenSSH public keys authorized for SSH access to this VM. */
            ssh_public_keys?: string[];
        };
        SshPublicKeyInfo: {
