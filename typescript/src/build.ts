 ...
 * later `RUN` is wrapped in `su -p`.
 *
 * Shell state is per-session, though, so neither mechanism reaches the sessions
 * the caller actually runs in. The final `USER` and `WORKDIR` are therefore also
 * written into the guest's image config by {@link persistConfig}, the file the
 * agent re-reads every time it starts a shell.
 *
 * ## COPY
 *
 ...
 */
const GUEST_CONFIG_PATH = "/etc/arker/image-config.json";
const GUEST_PROFILE_PATH = "/etc/profile.d/99-arker-image.sh";
const BUILD_SESSION_IDX = 0;

/**
 * Single-quote for POSIX sh, always, the way the agent's own writer does.
 * `shellQuote` leaves a "safe" value bare, which would make this file differ
 * from the one `arkerd` writes for the same values.
 */
function sqQuote(value: string): string {
...
type GuestConfig = { env?: [string, string][]; user?: string; workdir?: string };

/**
 * Re-render the prelude the agent sources into every shell. Byte-for-byte the
 * shape `arkerd` writes at conversion time: this file replaces that one, so a
 * different shape would be a silent divergence between an `image` fork and a
 * `dockerfile` fork of the same image.
 */
function renderProfile(config: GuestConfig): string {
  let out =
  ...
  for (const [key, value] of config.env ?? []) {
    if (isSafeEnvKey(key)) out += `export ${key}=${sqQuote(value)}\n`;
  }
  if (config.workdir) out += `cd ${sqQuote(config.workdir)} 2>/dev/null || true\n`;
  return out;
}
 ...
 * from a Dockerfile ending in `USER app` handed the caller a root shell — more
 * privilege than their own Dockerfile asked for — and a WORKDIR that
...
async function persistConfig(
  ...
  if (user === undefined && workdir === undefined) return;

  // Read through the file API rather than a shell: this is not a build step,
  // and it must not land in the caller's session history.
  let config: GuestConfig = {};
  try {
    const parsed = JSON.parse(new TextDecoder().decode(await vm.sync(GUEST_CONFIG_PATH)));
  ...
  await vm.sync(GUEST_CONFIG_PATH, JSON.stringify(config, null, 2));
  await vm.sync(GUEST_PROFILE_PATH, renderProfile(config));

  // The build's own shell is ALREADY running — as root, in its old cwd — and a
  // shell reads the config only when it starts. Dropping the session makes the
  // next run respawn it, so the VM the caller receives behaves the same in the
  // session the build used as in every other one.
  const listed = (await vm.listSessions()) as {
    sessions?: { session_id?: string; sessionId?: string; session_idx?: number; sessionIdx?: number }[];
  };
  for (const session of listed?.sessions ?? []) {
    const idx = session.sessionIdx !== undefined ? session.sessionIdx : session.session_idx;
    const id = session.sessionId ?? session.session_id;
    if (idx === BUILD_SESSION_IDX && id) await vm.deleteSession(id);
  }
}

/** Resolve a WORKDIR against the current one, the way Docker does. */
function joinWorkdir(current: string, next: string): string {
  if (next.startsWith("/")) return next;
  return nodePath.posix.normalize(nodePath.posix.join(current, next));
}
...
export async function applySteps(
  ...
  for (const step of steps) {
    ...
    switch (step.kind) {
      ...
      case "arg":
        ...
        break;
      case "workdir": {
        currentWorkdir = joinWorkdir(currentWorkdir ?? "/", step.path);
  ...
  await persistConfig(vm, currentUser, currentWorkdir);
}
